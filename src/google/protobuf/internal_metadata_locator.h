...
class InternalMetadataOffset {
 ...
 public:
  ...
  static constexpr PROTOBUF_ALWAYS_INLINE InternalMetadataOffset Build() {
    ...
    // Android local modification: support unsigned integer overflow sanitizer.
    int64_t result = 0;
    __builtin_sub_overflow(kInternalMetadataOffset, kFieldOffset, &result);
    return InternalMetadataOffset(static_cast<int32_t>(result));
  }
  ...
  static InternalMetadataOffset BuildFromDynamicOffset(size_t field_offset) {
    ...
    // Android local modification: support unsigned integer overflow sanitizer.
    int64_t result = 0;
    __builtin_sub_overflow(kInternalMetadataOffset, field_offset, &result);

    ABSL_DCHECK_GE(result, int64_t{INT32_MIN})
        << "Offset from `_internal_metadata_` to the field at offset "
        << field_offset
        << " is underflowing an int32_t, likely meaning your message body is "
           "too large.";
    ABSL_DCHECK_LE(result, int64_t{INT32_MAX})
        << "Offset from `_internal_metadata_` to the field at offset "
        << field_offset
